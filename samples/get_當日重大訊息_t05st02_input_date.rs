...
use std::error::Error;
use std::io::{self, Write}; // 🎯 引入標準輸入輸出模組
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // 🎯 核心新增：終端機動態輸入邏輯
    // ---------------------------------------------------------
    let mut query_year = String::new();
    let mut query_month = String::new();
    let mut query_day = String::new();

    println!("=== 🇹🇼 臺灣公開資訊觀測站 當日重訊查詢系統 ===");
    
    print!("👉 請輸入民國年 (例如 115): ");
    io::stdout().flush()?;
    io::stdin().read_line(&mut query_year)?;
    let query_year = query_year.trim();

    print!("👉 請輸入月份 (例如 08): ");
    io::stdout().flush()?;
    io::stdin().read_line(&mut query_month)?;
    let query_month = query_month.trim();

    print!("👉 請輸入日期 (例如 24): ");
    io::stdout().flush()?;
    io::stdin().read_line(&mut query_day)?;
    let query_day = query_day.trim();
    // ---------------------------------------------------------
