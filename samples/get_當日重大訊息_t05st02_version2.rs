...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // 🎯 核心日期指定：今天就是民國 115 年 08 月 24 日（週一交易日）
    let query_year = "115";   // 對應 id="year"
    let query_month = "08";  // 對應 id="month"
    let query_day = "24";    // 對應 id="day"
    ...
    println!("第二階段 ➔ 🚀 完全複製前端表單參數，發送核心 POST 請求...");
    
    // 🎯 終極修復核心：100% 複製您提供的 HTML 表單控制項鍵值對
    let mut params = HashMap::new();
    params.insert("step", "1");               // <input type="hidden" name="step" value="1">
    params.insert("step00", "0");             // <input type="hidden" name="step00" value="0">
    params.insert("firstin", "ture");         // 💡 注意：必須完全複製網頁原始碼的拼字錯誤 "ture"！
    params.insert("off", "1");                // <input type="hidden" name="off" value="1">
    params.insert("TYPEK", "all");            // <input type="hidden" name="TYPEK" value="all">
    
    // 注入動態指定的自訂年月日
    params.insert("year", query_year);        // 對應 name="year"
    params.insert("month", query_month);      // 對應 name="month"
    params.insert("day", query_day);          // 對應 name="day"
